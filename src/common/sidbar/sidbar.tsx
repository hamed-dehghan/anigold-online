...
export function AppSidebar({ ClickedItem, setClickedItem, location }: AppSidebarProps) {
  ...
  return (
    <Sidebar collapsible="icon" side="right" variant="floating"
      className=" !border-none mr-2 flex-shrink-0 rounded-[15px] bg-white text-colorTextSidbar "
    >
      <SidebarContent className="bg-white hide-scrollbar rounded-[15px]">
        <SidebarGroup >
          <SidebarGroupContent >
            <SidebarMenu >
      ...
      <SidebarFooter className={`${state === 'collapsed' ? 'hidden' : ' bg-white flex flex-row justify-start items-center py-5 pr-5 rounded-[15px]'}`}>
